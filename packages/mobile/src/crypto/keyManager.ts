...
/** Install a recovered master_key as the device's key (recovery path). */
...
export class EncryptingKVStore implements SecureKVStore {
  ...
  private constructor(private inner: SecureKVStore, storeKeyPromise: Promise<Uint8Array>) {
  ...
  static lazy(inner: SecureKVStore, getMasterKey: () => Promise<Uint8Array>): EncryptingKVStore {
  ...
  async get(key: string): Promise<string | null> {
    ...
    return kb.openBlob(await this.storeKeyPromise, v); // null on undecryptable → treated as absent
  }
