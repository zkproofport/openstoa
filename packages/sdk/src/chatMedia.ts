 ...
 *      decrypts it locally.
 *
 * Two copies exist — `src/lib/chatMedia.ts` (web/server) and
 * `packages/mobile/src/lib/chatMedia.ts` (mini-app) — and a test asserts they
...
export interface ChatMediaSendDeps {
  ...
  upload(ciphertextB64: string, mediaId: string): Promise<string>;
...
export async function sendEncryptedChatMedia(
  ...
  const sealed = await deps.seal(mediaId, bytes);
  if (!sealed) throw new ChatMediaError('no-key');

  let objectKey: string;
  try {
    objectKey = await deps.upload(bytesToBase64(sealed.ciphertext), mediaId);
  } catch (err) {
...
/** A `data:` URI for a decrypted attachment — how React Native displays it. */
export function chatMediaDataUri(bytes: Uint8Array, mime: string): string {
