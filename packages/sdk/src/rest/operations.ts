...
const paging = [query('limit', 'Maximum results (1–100)', { type: 'number', min: 1, max: 100 }), query('offset', 'Pagination offset', { type: 'number', min: 0 })];
...
export const REST_OPERATIONS: readonly RestOperation[] = [
  op('upload_delete', ['upload-delete'], 'DELETE', '/api/upload', 'Delete uploaded images owned by your account; foreign URLs are skipped.', [body('urls', 'Comma-separated upload URLs', { type: 'strings' })], { access: 'Authenticated upload owner; foreign or invalid URLs are skipped.' }),
  op('feed', ['feed'], 'GET', '/api/feed', 'Read or search the cross-topic feed.', [search, ...paging, query('sort', 'Sort order', { choices: ['hot', 'new', 'top', 'active'] }), query('tag', 'Tag slug'), query('category', 'Category slug'), query('view', 'my limits to joined topics', { choices: ['my'] })], { auth: 'public', access: 'Guests see public topics; signed-in accounts also see joined topics.' }),
  ...
  op('activity_recorded', ['activity', 'recorded'], 'GET', '/api/my/recorded', 'Read or search your on-chain records.', [search, ...paging]),
  op('activity_recorded_on_mine', ['activity', 'recorded-on-mine'], 'GET', '/api/my/recorded-on-mine', 'Read or search records others made of your posts.', [search, ...paging]),
  ...
  op('dm_candidates', ['dm', 'candidates'], 'GET', '/api/dm/candidates', 'Find people to start a conversation with.', [search, query('limit', 'Maximum candidates', { type: 'number', min: 1, max: 500 })], { capabilities: ['/openstoa/chat/read'] }),
  op('post_vote', ['post', 'vote'], 'POST', '/api/posts/{postId}/vote', 'Toggle or change a post vote; repeating the same value removes it.', [postId, body('value', '1 for upvote, -1 for downvote', { type: 'number', choices: [1, -1] })], { access: 'Topic member.' }),
  op('post_bookmark', ['post', 'bookmark'], 'POST', '/api/posts/{postId}/bookmark', 'Toggle a post bookmark.', [postId], { access: 'Topic member.' }),
  ...
  op('post_record', ['post', 'record'], 'POST', '/api/posts/{postId}/record', 'Record a post on-chain; eligibility and daily limits apply.', [postId], { access: 'Topic member; not your own post; at least one hour old; daily limit.' }),
  ...
  op('post_react', ['post', 'react'], 'POST', '/api/posts/{postId}/reactions', 'Toggle an emoji reaction.', [postId, body('emoji', 'Reaction emoji', { choices: ['👍', '❤️', '🔥', '😂', '🎉', '😮'] })], { access: 'Topic member.' }),
  ...
  op('post_poll_vote', ['post', 'poll-vote'], 'POST', '/api/posts/{postId}/poll/vote', 'Vote in a poll.', [postId, body('optionIds', 'Comma-separated poll option IDs', { type: 'strings' })], { access: 'Topic member; poll must be open.' }),
  op('post_poll_unvote', ['post', 'poll-unvote'], 'DELETE', '/api/posts/{postId}/poll/vote', 'Remove your vote from an open poll.', [postId]),
  op('topic_delete', ['topics', 'delete'], 'DELETE', '/api/topics/{topicId}', 'Delete a topic you own or administer.', [topicId], { access: 'Topic owner/creator or site administrator; personal topics cannot be deleted here.' }),
  ...
  op('topic_requests', ['topics', 'requests'], 'GET', '/api/topics/{topicId}/requests', 'List legacy topic join requests.', [topicId, query('status', 'Request status', { choices: ['pending', 'all'] })], { access: 'Topic owner or admin.' }),
  op('topic_approve', ['topics', 'approve'], 'PATCH', '/api/topics/{topicId}/requests', 'Approve a legacy topic join request.', [topicId, body('requestId', 'Request ID')], { body: { action: 'approve' }, access: 'Topic owner or admin.' }),
  ...
  op('profile_domain_badge', ['profile', 'domain-badge'], 'GET', '/api/profile/domain-badge', 'Read your public organization domains.', [], { capabilities: ['/openstoa/profile/read'] }),
  op('profile_set_domain_badge', ['profile', 'set-domain-badge'], 'POST', '/api/profile/domain-badge', 'Publish domains from valid workspace verifications.', [], { capabilities: ['/openstoa/profile/edit'] }),
  op('profile_remove_domain_badge', ['profile', 'remove-domain-badge'], 'DELETE', '/api/profile/domain-badge', 'Hide an organization domain, or all domains when omitted.', [body('domain', 'Domain to hide; omit for all', { required: false })], { capabilities: ['/openstoa/profile/edit'] }),
  ...
  op('chat_mark_read', ['chat', 'mark-read'], 'PUT', '/api/topics/{topicId}/chat/read', 'Mark a message as read (account-level cursor).', [topicId, body('messageId', 'Server message ID'), body('readAt', 'ISO timestamp of the read')], { capabilities: ['/openstoa/chat/read'], access: 'Topic member.' }),
];
