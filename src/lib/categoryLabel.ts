/** Built-in categories are UI copy; unknown/custom categories keep their name. */
...
export function categoryLabel(category: { slug: string; name: string }, t: (key: string) => string): string {
  return BUILT_IN_CATEGORIES.has(category.slug) ? t(`webUi.categories.${category.slug}`) : category.name;
}
